...
export default class SessionDatabase extends AbstractAdapterSession {
    static async read(request, options) {
        ...
        if (!request.cookies[config.name])
            return this.create();
        ...
        const sid = seg[0];
        const sign = seg[1];
        const verify = await HelperCrypto.verify(JSON.parse(Central.adapter.process().env.SESSION_SECRET), sign, sid);
        if (!verify) {
            return this.create();
        }
    ...
    static async write(request, cookies, options) {
        ...
        await model.write();
        const sign = await HelperCrypto.sign(JSON.parse(Central.adapter.process().env.SESSION_SECRET), model.sid);
        const cookieName = `${model.sid}.${sign}`;
        //if session cookie is same, no need to set cookie
        if (!request.session.id)
            request.session.id = model.id;
        cookies.push({
            name: config.name,
            value: cookieName,
